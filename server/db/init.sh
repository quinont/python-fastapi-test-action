...
create_table() {
	PGPASSWORD="$WORKSHOP_POSTGRES_PASSWORD" psql -v ON_ERROR_STOP=1 -U "$WORKSHOP_POSTGRES_USER" -d "$WORKSHOP_POSTGRES_DB" <<-EOSQL
        CREATE TABLE books (
...
connect_database() {
	PGPASSWORD="$WORKSHOP_POSTGRES_PASSWORD" psql -U "$WORKSHOP_POSTGRES_USER" -d "$WORKSHOP_POSTGRES_DB"
}
